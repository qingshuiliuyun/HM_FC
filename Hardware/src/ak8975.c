/******************** (C) COPYRIGHT 2014 ANO Tech ********************************
  * 作者   ：匿名科创
 * 文件名  ：ak8975.c
 * 描述    ：电子罗盘驱动
 * 官网    ：www.anotc.com
 * 淘宝    ：anotc.taobao.com
 * 技术Q群 ：190169595
**********************************************************************************/

#include "ak8975.h"
#include "parameter.h"
#include "senser.h"
//#include "mymath.h"

#include "include.h"
#include "i2c.h"

// 	xyz_f_t Mag_bias = { -1 , -1 , -1 };
// 	xyz_f_t Mag_Gain   = { 1 , 0.8538 , 0.9389 };

// ak8975_t ak8975 = { {0,0,0},{-1,-1,-1},{1,0.8538,0.9389},{0,0,0} };

bool AK8975_Run(void)
{
	return IIC_Write_1Byte(AK8975_ADDRESS,AK8975_CNTL,0x01);	
}

_xyz_f XYZ_STRUCT_COPY(float x,float y, float z)
{
	_xyz_f m ;
	m.x = x;
	m.y = y;
	m.z = z;
	return m;
}
u8 ak8975_ok;
void AK8975_Read_Mag_Data(void)
{
	int16_t mag_temp[3];
	u8 ak8975_buffer[6]; //接收数据缓存
	
	//I2C_FastMode = 0;
	
	IIC_Read_1Byte(AK8975_ADDRESS,AK8975_HXL,&ak8975_buffer[0]); 
	IIC_Read_1Byte(AK8975_ADDRESS,AK8975_HXH,&ak8975_buffer[1]);
	mag_temp[1] = ((((int16_t)ak8975_buffer[1]) << 8) | ak8975_buffer[0]) ;  //磁力计X轴

	IIC_Read_1Byte(AK8975_ADDRESS,AK8975_HYL,&ak8975_buffer[2]);
	IIC_Read_1Byte(AK8975_ADDRESS,AK8975_HYH,&ak8975_buffer[3]);
	mag_temp[0] = -((((int16_t)ak8975_buffer[3]) << 8) | ak8975_buffer[2]) ;  //磁力计Y轴

	IIC_Read_1Byte(AK8975_ADDRESS,AK8975_HZL,&ak8975_buffer[4]);
	IIC_Read_1Byte(AK8975_ADDRESS,AK8975_HZH,&ak8975_buffer[5]);
	mag_temp[2] = ((((int16_t)ak8975_buffer[5]) << 8) | ak8975_buffer[4]) ;  //磁力计Z轴	
	
	Senser.AHRS.mag.raw.x = mag_temp[0];
	Senser.AHRS.mag.raw.y = mag_temp[1];
	Senser.AHRS.mag.raw.z = mag_temp[2];
	
	Senser.AHRS.mag.real.x = (Senser.AHRS.mag.raw.x - Senser.AHRS.mag.bias.x) ;
	Senser.AHRS.mag.real.y = (Senser.AHRS.mag.raw.y - Senser.AHRS.mag.bias.y) ;
	Senser.AHRS.mag.real.z = (Senser.AHRS.mag.raw.z - Senser.AHRS.mag.bias.z) ;
	//磁力计中点矫正	
	AK8975_CalOffset_Mag();
	
	//AK8975采样触发
	AK8975_Run();
}

_xyz_s AK8975_Get_Mag(void)
{
	return Senser.AHRS.mag.real;
}

//磁力计中点矫正

void AK8975_CalOffset_Mag(void)
{
	static _xyz_f	MagMAX = { -100 , -100 , -100 }, MagMIN = { 100 , 100 , 100 };// MagSum;
	static uint16_t cnt_m=0;
	
	if(Senser.AHRS.mag.isGetOffset)
	{	
		
		if(ABS(Senser.AHRS.mag.raw.x)<400&&ABS(Senser.AHRS.mag.raw.y)<400&&ABS(Senser.AHRS.mag.raw.z)<400)
		{
			MagMAX.x = MAX(Senser.AHRS.mag.raw.x, MagMAX.x);
			MagMAX.y = MAX(Senser.AHRS.mag.raw.y, MagMAX.y);
			MagMAX.z = MAX(Senser.AHRS.mag.raw.z, MagMAX.z);
			
			MagMIN.x = MIN(Senser.AHRS.mag.raw.x, MagMIN.x);
			MagMIN.y = MIN(Senser.AHRS.mag.raw.y, MagMIN.y);
			MagMIN.z = MIN(Senser.AHRS.mag.raw.z, MagMIN.z);		
			
			if(Senser.AHRS.mag.isGetOffsetOK)
			{
				Senser.AHRS.mag.bias.x = (int16_t)((MagMAX.x + MagMIN.x) * 0.5f);
				Senser.AHRS.mag.bias.y = (int16_t)((MagMAX.y + MagMIN.y) * 0.5f);
				Senser.AHRS.mag.bias.z = (int16_t)((MagMAX.z + MagMIN.z) * 0.5f);
	
// 				MagSum.x = MagMAX.x - MagMIN.x;
// 				MagSum.y = MagMAX.y - MagMIN.y;
// 				MagSum.z = MagMAX.z - MagMIN.z;
				
// 				ak8975.Mag_Gain.y = MagSum.x / MagSum.y;
// 				ak8975.Mag_Gain.z = MagSum.x / MagSum.z;
				
				Parameter_SaveOffset();
				
				Senser.AHRS.mag.isGetOffset   = 0;
				Senser.AHRS.mag.isGetOffsetOK = 0;
				cnt_m =0;
			}
		}
		cnt_m++;
		
	}
	else
	{

	}
}

void AK8975_Read(void)
{
		//读取磁力计
		AK8975_Read_Mag_Data();
}


/******************* (C) COPYRIGHT 2014 ANO TECH *****END OF FILE************/

